...
Class API {
	...
	public static function locate_template( $tpl ) {
		...
		$located = locate_template( array( '/views/' . BEA_WM_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ), false, false );
		if ( ! empty( $located ) ) {
			return $located;
		}

		// Locate on the files
		if ( is_file( BEA_WM_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( BEA_WM_DIR . 'views/' . $tpl . '.php' );
		}
